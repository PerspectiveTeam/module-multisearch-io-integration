...
namespace Perspective\MultisearchIo\Model\ResourceModel\MultisearchAutocomplete;
...
use Perspective\MultisearchIo\Model\Autocomplete\Config\Source\CategoryLinkType;
use Perspective\MultisearchIo\Model\MultisearchAutocomplete as Model;
use Perspective\MultisearchIo\Model\ResourceModel\MultisearchAutocomplete as ResourceModel;
...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
     ...
     */
    protected $_eventPrefix = 'pst_ms_autocomplte_collection';
     ...
     * @param TotalRecordsResolverFactory $totalRecordsResolverFactory
    ...
    public function __construct(
        ...
        private ?DbStorage $urlFinder = null,
    ...
    public function addDeletionFlag()
    {
        $this->filterBuilder->setField('is_deletion_term');
        $this->filterBuilder->setValue(true);
     ...
     * Він працює лише queryText НЕ встановлено
     * @return array
     */
    public function getHistory()
    {
        if (empty($this->queryText) || $this->queryText === RequestInterface::PARAM_QUERY_EMPTY) {
            $this->_renderFilters();
            return method_exists($this->searchResult, 'getHistory') ? $this->searchResult->getHistory() : [];
        } else {
            return [];
        }
    }
    ...
    {
        $this->_renderFilters();
        $categoryBucket = $this->searchResult->getAggregations()?->getBucket('category' . RequestGenerator::BUCKET_SUFFIX);
        $bucketValues = $categoryBucket ? $categoryBucket->getValues() : [];

        $categoryIds = [];
        foreach ($bucketValues as $value) {
            $categoryIds[] = $value->getValue();
        }
        $categories = $this->catalogUrl->getCategories($categoryIds, $this->storeManager->getStore()->getId()) ?: [];

        if ($this->getLinkType() === CategoryLinkType::MULTISEARCH_IO_CATEGORY_ID_PLUS_QUERY) {
            foreach ($categories as $category) {
                $categoryPath = 'multisearch/search/index';
                $query = $this->queryText && $this->queryText !== RequestInterface::PARAM_QUERY_EMPTY ? $this->queryText : '';
                $category->setUrl($this->storeManager->getStore()->getUrl($categoryPath, ['_query' => ['category_ids' => $category->getId(), 'q' => $query]]));
            }
        } else {
            foreach ($categories as $category) {
                ...
                $category->setUrl($this->storeManager->getStore()->getBaseUrl() . $categoryPath);
            }
        }
    ...
    {
        return $this->_scopeConfig->getValue('perspective_multisearch_io/autocomplete/category_link_type');
    }
