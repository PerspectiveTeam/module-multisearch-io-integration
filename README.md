...
### Прямі переходи (Direct)
*   Підтримує отримання `direct`-даних від Multisearch.io для сценаріїв прямого переходу за пошуковим запитом.
*   Сумісний як зі старим форматом `direct` у вигляді рядка або об'єкта з `url`, так і з новим форматом об'єкта з `url`, `id`, `type`.
*   Для суперечливих значень пріоритетною ціллю переходу вважається `url`, якщо вона передана.
